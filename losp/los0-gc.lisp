...
(defun make-space (location size)
  ...
  (macrolet ((x (index)
    ...
    (setf (x 1) (* #.movitz:+movitz-fixnum-factor+ size)
	  (x 0) #.(cl:dpb (movitz::enum-value 'movitz:movitz-vector-element-type :u32)
			  (cl:byte 8 8)
			  (movitz::enum-value 'movitz:other-type-byte :basic-vector))))
...
;;             no-check
	    ...
	    (:locally (:movl ,(movitz::atomically-continuation-simple-pf 'fast-cons)
            ...
            (:locally (:movl (:edi (:edi-offset nursery-space)) :edx))
	    (:movl (:edx 2) :ecx)
	    (:cmpl (:edx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::num-elements))
		   :ecx)
	...
	(macrolet
	    ((do-it ()
		 ...
		 retry
		  ...
		  (:addl 4 :eax)
		  (:andl -8 :eax)
		  (:movl (:edx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::num-elements))
			 :ecx)
		  (:subl :eax :ecx)
		  (:movl (:edx 2) :ebx)
		  (:cmpl :ecx :ebx)
