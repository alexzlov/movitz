	...
	((>= scan end-location))
      ...
      (let ((x (memref scan 0 :type :unsigned-byte16))
	   ...
	   (let ((rtc (%word-offset scan #.(movitz:tag :other))))
	     (incf scan)
	     (let ((non-lispvals #.(cl:truncate (cl:+ -4 (movitz::slot-offset 'movitz::movitz-run-time-context
		...
		(let ((num-jumpers (memref scan 0 :type :unsigned-byte14)))
          ...
          ((scavenge-typep x :basic-vector)
           ...
           (cond
             ...
             ((scavenge-wide-typep x :basic-vector #.(movitz::enum-value 'movitz:movitz-vector-element-type :u16))
		  ...
		  (scavenge-wide-typep x :basic-vector
             ...
             ((scavenge-wide-typep x :basic-vector #.(movitz::enum-value 'movitz:movitz-vector-element-type :bit))
              (let ((len (memref scan 4)))
                (record-scan :other)
                (incf scan (1+ (* 2 (truncate (+ 63 len) 64))))))
             ((or (scavenge-wide-typep x :basic-vector
                                       #.(movitz::enum-value 'movitz:movitz-vector-element-type
							:any-t))
                  (scavenge-wide-typep x :basic-vector
                                       #.(movitz::enum-value 'movitz:movitz-vector-element-type
							:indirects)))
