  ...
  (macrolet
	 ...
	 `(with-inline-assembly (:returns :eax)
	   ...
	   shrink-loop
	    (:cmpl 4 :ecx)
	    (:je 'shrink-no-more)
	    (:cmpl 0 (:eax :ecx ,(+ -4 (movitz::slot-offset 'movitz:movitz-bignum 'movitz::bigit0))))
	    (:jnz 'shrink-done)
	    (:subl 4 :ecx)
	    (:jmp 'shrink-loop)
	   shrink-no-more
	    (:cmpl ,(1+ movitz:+movitz-most-positive-fixnum+)
		   (:eax ,(movitz::slot-offset 'movitz:movitz-bignum 'movitz::bigit0)))
	   ...
	   shrink-done
	    ...
	    (:movw :cx (:eax ,(movitz::slot-offset 'movitz:movitz-bignum 'movitz::length)))
	   done
	    )))
    (do-it)))
...
(defun bignum-addf-fixnum (bignum delta)
  ...
  (macrolet
	 ...
	 `(with-inline-assembly (:returns :eax :labels (add-bignum-loop add-bignum-done))
	    ...
	    (:movzxw (:eax ,(movitz::slot-offset 'movitz::movitz-bignum 'movitz::length)) :ebx) ; length
	    (:xorl :edx :edx)		; counter
	    (:sarl ,movitz:+movitz-fixnum-shift+ :ecx)
	    (:jns 'positive-delta)
	    ;; negative-delta
	    (:negl :ecx)
	    (:subl :ecx (:eax ,(movitz::slot-offset 'movitz::movitz-bignum 'movitz::bigit0)))
	    (:jnc 'add-bignum-done)
	   sub-bignum-loop
	    (:addl 4 :edx)
	    (:cmpl :edx :ebx)
	    (:je '(:sub-program (overflow) (:int 4)))
	    (:subl 1 (:eax :edx ,(movitz::slot-offset 'movitz::movitz-bignum 'movitz::bigit0)))
	    (:jc 'sub-bignum-loop)
	    (:jmp 'add-bignum-done)
	    
	   positive-delta
	    (:addl :ecx (:eax ,(movitz::slot-offset 'movitz::movitz-bignum 'movitz::bigit0)))
	    (:jnc 'add-bignum-done)
	   add-bignum-loop
	    (:addl 4 :edx)
	    (:cmpl :edx :ebx)
	    (:je '(:sub-program (overflow) (:int 4)))
	    (:addl 1 (:eax :edx ,(movitz::slot-offset 'movitz::movitz-bignum 'movitz::bigit0)))
	    (:jc 'add-bignum-loop)
	   add-bignum-done)))
    (do-it)))
