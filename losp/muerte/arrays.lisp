...
(defmacro/cross-compilation vector-double-dispatch ((s1 s2) &rest clauses)
  (flet ((make-double-dispatch-value (et1 et2)
	   (+ (* #x100 (movitz::enum-value 'movitz::movitz-vector-element-type et1))
	      (movitz::enum-value 'movitz::movitz-vector-element-type et2))))
...
(define-compiler-macro vector-element-type-code (object)
  `(let ((x (memref ,object (movitz-type-slot-offset 'movitz-basic-vector 'element-type)
		    :type :unsigned-byte8)))
     (if (/= x ,(movitz::enum-value 'movitz::movitz-vector-element-type :indirects))
	 x
...
(defun array-element-type (array)
  (ecase (vector-element-type-code array)
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :any-t)
       t)
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :character)
       'character)
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :u8)
       '(unsigned-byte 8))
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :u16)
       '(unsigned-byte 16))
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :u32)
       '(unsigned-byte 32))
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :bit)
       'bit)
    (#.(movitz::enum-value 'movitz::movitz-vector-element-type :code)
       'code)))
...
(define-compiler-macro %basic-vector-has-fill-pointer-p (vector)
  ...
  `(with-inline-assembly (:returns :boolean-zf=1)
     ...
     (:testl ,(logxor #xffffffff (* movitz:+movitz-fixnum-factor+ (1- (expt 2 14))))
	     (:eax ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::num-elements)))))
...
%basic-vector-has-fill-pointer-p is true."
  `(with-inline-assembly (:returns :register)
     ...
     (:movzxw ((:result-register)
	       ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::fill-pointer))
	      (:result-register))))
...
(defun shallow-copy-vector (vector)
  ...
  (let ((length (the fixnum
    ...
    (ecase (memref vector (movitz-type-slot-offset 'movitz-basic-vector 'element-type)
		   :type :unsigned-byte8)
      ((#.(movitz::enum-value 'movitz::movitz-vector-element-type :any-t)
	#.(movitz::enum-value 'movitz::movitz-vector-element-type :indirects))
       (%shallow-copy-object vector (+ 2 length)))
      ((#.(movitz::enum-value 'movitz::movitz-vector-element-type :u32)
	  #.(movitz::enum-value 'movitz::movitz-vector-element-type :stack))
	 ...
	 (macrolet
		...
		`(with-inline-assembly (:returns :eax)
		   (:declare-label-set
			...
			do (setf (elt x (movitz::enum-value
		   ...
		   (:jmp (:esi (:ecx 4) 'basic-vector-dispatcher
			       ,(movitz::slot-offset 'movitz:movitz-funobj 'movitz::constant0)))
		   
		   (:jnever '(:sub-program (unknown)
			      (:int 100)))
		  :u32
		  :stack
		   (:movl (:eax :ebx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data))
			  :ecx)
		  ...
		  :u8 :code
		   (:movl :ebx :ecx)
		   (:shrl ,movitz:+movitz-fixnum-shift+ :ecx)
		   (:movzxb (:eax :ecx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data))
			    :ecx)
		  ...
		  :character
		   ...
		   (:movb (:ebx :ecx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data))
			  :ah)
		   (:jmp 'return)
		  :bit
		   (:movl :ebx :ecx)
		   ...
		   (:shrl ,movitz:+movitz-fixnum-shift+ :ecx)
		   (:xorl :eax :eax)
		   (:btl :ecx (:ebx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data)))
		   (:jnc 'return)
	 ...
	 (macrolet
		...
		`(with-inline-assembly (:returns :eax)
		   ...
		   (:jnz '(:sub-program (not-an-index)
			   ...
			   (:int 59)))
		   (:cmpl (:ebx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::num-elements))
			  :edx)
		   ...
		   (,movitz:*compiler-nonlocal-lispval-write-segment-prefix*
		    :movl :eax
			  (:ebx :edx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data)))
		   (:jmp 'return)
		  ...
		  not-any-t-vector
		   ...
		   (:movl :edx :ecx)
		   (:shrl ,movitz:+movitz-fixnum-shift+ :ecx)
		   (:movb :ah (:ebx :ecx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data)))
		   (:jmp 'return)
		  ...
		  not-u32-vector
		   ...
		   (:testl :eax :eax)
		   (:jnz 'set-one-bit)
		   (:btrl :ecx (:ebx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data)))
		   (:jmp 'return)
		  set-one-bit
		   (:btsl :ecx (:ebx ,(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data)))
		   (:jmp 'return)
...
;;  (compiler-macro-call svref%unsafe simple-vector index))
  (with-inline-assembly (:returns :eax)
    ...
    (:movl (:eax :ebx #.(movitz::slot-offset 'movitz:movitz-basic-vector 'movitz::data)) :eax)))
	...
	`(let ((,var ,vector))
	   (if (not (typep ,var 'vector-u8))
	     ...
	     (with-inline-assembly (:returns :untagged-fixnum-ecx)
	       ...
	       (:cmpl (:eax ,(movitz::slot-offset 'movitz::movitz-basic-vector
					     'movitz::num-elements))
		      :ecx)
	       (:jnc '(:sub-program () (:int 69)))
	       (:shrl ,movitz::+movitz-fixnum-shift+ :ecx)
	       (:movw (:eax :ecx ,(+ actual-index (movitz::slot-offset 'movitz::movitz-basic-vector
								  'movitz::data)))
		      :cx)
	       (:xchgb :cl :ch))))))))
